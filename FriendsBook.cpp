...
void join(MyADT &theMembers)
{
    ...
    Profile testMember(theUserName);     // Profile* testMember = new Profile(theUserName); both are same but chalaite hoile . er bodole -> lagbe
    ...
    if (testMember.getUserName() == Profile::getDefaultStrValue())
        cout << "'" << theUserName << "'"
             << " is not a valid userName. A userName must have a lower case letter as its first character. Please, try again!." << endl;
    else
    {
        ...
        if (target == nullptr)
        {
            ...
            if (theMembers.insert(theMember))
                // Report to user
                cout << "Member: "
                     << "'" << theMember.getUserName() << "'"
                     << " has successfully joined FriendsBook." << endl;
            else
                cout << "Member: "
                     << "'" << theMember.getUserName() << "'"
                     << " was unable to joined FriendsBook (she/he may already be a member?)" << endl;
        }
        else
            cout << "Member: "
    ...
    else
    {
        ...
        if (theMembers.remove(toBeRemoved))
            cout << "Friend : "
                 ...
                 << " has now left this social network." << endl;
        else
            cout << "Friend : "
                 << "'" << toBeRemoved.getUserName() << "'"
                 << " is not a member of this social network!" << endl;
    }
    return;
}
...
void search(MyADT &theMembers)
{
    ...
    else
    {
        ...
        if (target != nullptr)
            ...
            cout << "Member: "
                 << "'" << target->getUserName() << "'"
                 << " has been successfully found in FriendsBook." << endl;
        else
            cout << "'" << theUserName << "'"
                 << " is not a member of FriendsBook." << endl;
    }
...
void modify(MyADT &theMembers)
{
    ...
    else
    {
        ...
        {
            // Modify
            cout << "Modifying member's name "
                 << "'" << target->getName() << "'" << endl;
            cout << "Please enter new name for this profile (press ENTER to skip): ";
            getline(cin, newString);
            if (!newString.empty())
                target->setName(newString);

            cout << "Modifying member's email "
                 << "'" << target->getEmail() << "'" << endl;
            cout << "Please, enter the new email of this member (press ENTER to skip): ";
            getline(cin, newString);
            if (!newString.empty())
                target->setEmail(newString);

            cout << "Modifying member's birthday "
                 << "'" << target->getBirthday() << "'" << endl;
            cout << "Please, enter the new birthday of this member(press ENTER to skip): ";
            getline(cin, newString);
            if (!newString.empty())
                target->setBirthday(newString);
        }
        else
            cout << "Member : "
                 << "'" << theMember.getUserName() << "'"
...
int main()
{
    ...
    while (!done)
    {
        ...
        cout << "x -> to exit FriendsBook." << endl
             << endl;
